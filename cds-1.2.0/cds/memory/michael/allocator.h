...
#       error "Unexpected value of CDS_BUILD_BITS"
    ...
    protected:
        ...
        block_header * alloc_from_active( processor_heap * pProcHeap )
        {
            ...
            do {
                ++nCollision    ;
                newActive = oldActive = pProcHeap->active.load(CDS_ATOMIC::memory_order_acquire) ;
                if ( !oldActive.ptr() )
            ...
            } while ( !pProcHeap->active.compare_exchange_strong( oldActive, newActive, CDS_ATOMIC::memory_order_release, CDS_ATOMIC::memory_order_relaxed ) ) ;
