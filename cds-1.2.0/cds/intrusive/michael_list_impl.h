...
#   endif
    ...
    public:
        ...
        {
            static_assert( (boost::is_same< gc, typename node_type::gc >::value), "GC and node_type::gc must be the same type" ) ;
        }
        ...
        */
        ...
        /// Unlink the item \p val from the list
        /**
        ...
        */
        ...
        {
            return erase_at( m_pHead, val, func )    ;
        }
    ...
    protected:
        ...
        template <typename Q, typename Func>
        bool erase_at( atomic_node_ptr& refHead, const Q& val, Func f, position& pos )
        {
            back_off bkoff  ;
            while ( search( refHead, val, pos ) ) {
                if ( unlink_node( pos ) ) {
        ...
        template <typename Q, typename Func>
        bool erase_at( atomic_node_ptr& refHead, const Q& val, Func f )
        {
            position pos    ;
            return erase_at( refHead, val, f, pos )    ;
        }

        template <typename Q>
        bool erase_at( atomic_node_ptr& refHead, const Q& val )
        {
            position pos    ;
            return erase_at( refHead, val, empty_erase_functor(), pos )    ;
        }

        template <typename Q, typename Func>
        bool find_at( atomic_node_ptr& refHead, Q& val, Func f )
        {
            position pos    ;
        ...
        template <typename Q>
        struct find_at_functor {
            void operator()( value_type& item, Q const & val )
            {}
        };

        template <typename Q>
        bool find_at( atomic_node_ptr& refHead, Q const & val )
        {
            return find_at( refHead, val, find_at_functor<Q>() )    ;
        }
    ...
    protected:
        ...
        //@cond
        template <typename Q>
        bool search( atomic_node_ptr& refHead, const Q& val, position& pos )
        {
...
try_again:
            ...
            pos.guards.assign( 1, node_traits::to_value_ptr( pCur.ptr() ) )    ;
            ...
            while ( true ) {
                ...
                pNext = pCur->m_pNext.load(memory_model::memory_order_relaxed)   ;
                pos.guards.assign( 0, node_traits::to_value_ptr( pNext.ptr() )) ;
                ...
                if ( pNext.bits() == 1 ) {
                    ...
                    pPrev = &( pCur->m_pNext ) ;
                    pos.guards.assign( 2, node_traits::to_value_ptr( pCur.ptr() ) ) ;
                }
                pCur = pNext ;
                pos.guards.assign( 1, node_traits::to_value_ptr( pNext.ptr() )) ;
            }
        }
        //@endcond
    };
