...
#   endif
    ...
    protected:
        ...
        void unlink_node( node_type * pPred, node_type * pCur )
        {
            ...
            pCur->m_pNext.store( marked_node_ptr( head(), 1 ), memory_model::memory_order_release )    ;   // // back-link for search
        }
    ...
    protected:
        ...
        template <typename Q, typename Func>
        bool erase_at( node_type * pHead, const Q& val, Func f, position& pos )
        {
            key_comparator  cmp ;
        ...
        template <typename Q, typename Func>
        bool erase_at( node_type * pHead, const Q& val, Func f )
        {
            position pos    ;
            return erase_at( pHead, val, f, pos )    ;
        }

        template <typename Q>
        bool erase_at( node_type * pHead, const Q& val )
        {
            position pos    ;
            return erase_at( pHead, val, empty_erase_functor(), pos )    ;
        }

        template <typename Q, typename Func>
        bool find_at( node_type * pHead, Q& val, Func f )
        {
            position pos    ;

            search( pHead, val, pos )   ;
            if ( pos.pCur != tail() ) {
                cds::lock::Auto< typename node_type::lock_type> al( pos.pCur->m_Lock )  ;
                if ( !pos.pCur->is_marked()
                    && key_comparator()( *node_traits::to_value_ptr( *pos.pCur ), val ) == 0 )
                {
        ...
        template <typename Q>
        bool find_at( node_type * pHead, Q const & val )
        {
            position pos    ;
    ...
    protected:
        //@cond
        template <typename Q>
        void search( node_type * pHead, const Q& key, position& pos )
        {
            ...
            while ( pCur.ptr() != pTail )
            {
                ...
                pos.guards.assign( 0, node_traits::to_value_ptr( pCur.ptr() )) ;
                pPrev = pCur    ;
