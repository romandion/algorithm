    ...
    namespace michael_deque
    {
        ...
        */
        struct anchor
        {
            ...
            /// Default ctor initializes left/right indices with 0
            anchor() CDS_NOEXCEPT
                : idxLeft(0)
                , idxRight(0)
            {
        ...
        private:
            ...
            static void static_check()
            {
                ...
                //static_assert( alignof(anchor) == sizeof(unsigned int) * 2, "The anchor type must be properly aligned") ;
            }
            //@endcond
        };
    ...
    */
    ...
    private:
        ...
        typedef typename boost::mpl::if_<
            boost::is_same<typename options::stat, cds::intrusive::deque_stat<> >
            ,michael_deque::stat<>
            ,typename boost::mpl::if_<
                boost::is_same<typename options::stat, cds::intrusive::deque_dummy_stat>
    ...
    private:
        //@cond
        typedef michael_deque::anchor  anchor_type ;
        ...
        void stabilize_front( anchor_type& a )
        {
            ...
            unsigned int idxPrevNext = index( prevLinks.idxLeft ) ;
            if ( idxPrevNext != index( a.idxLeft ) ) {
                if ( m_Anchor.load( memory_model::memory_order_acquire ) != a )
                    return ;
        ...
        void stabilize_back( anchor_type& a )
        {
            ...
            anchor_type prevLinks = pPrev->m_Links.load( memory_model::memory_order_acquire ) ;
            unsigned int idxPrevNext = index( prevLinks.idxRight ) ;
            if ( idxPrevNext != index( a.idxRight ) ) {
                if ( m_Anchor.load( memory_model::memory_order_acquire ) != a )
                    return ;
    ...
    public:
        ...
        */
        MichaelDeque()
            :m_Anchor( anchor_type( c_nEmptyIndex, c_nEmptyIndex ))
            ,m_Mapper( 4096, 4 )
        {
            ...
            static_assert(( !boost::is_same<gc, cds::gc::HRC>::value ), "cds::gc::HRC is not allowed here")   ;
        }
