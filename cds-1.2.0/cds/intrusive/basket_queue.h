    ...
    */
    ...
    public:
        ...
        typedef typename boost::mpl::if_<
            boost::is_same<typename options::stat, cds::intrusive::queue_stat<> >
            ,basket_queue::stat<>
            ,typename boost::mpl::if_<
                boost::is_same<typename options::stat, cds::intrusive::queue_dummy_stat>
    ...
    protected:
        ...
        typename opt::details::alignment_setter< node_type, options::alignment >::type m_Dummy ;           ///< dummy node
        ...
        //@cond
        // GCC: m_Dummy is unaccessible in MoirQueue. Why?..
        node_type * dummy_node_ptr()
        {
            return &m_Dummy  ;
        }
        //@endcond
            ...
            while ( true ) {
                ...
                if ( h == m_pHead.load( memory_model::memory_order_acquire ) ) {
                    if ( h.ptr() == t.ptr() ) {
                        ...
                        while ( pNext.bits() && iter.ptr() != t.ptr() && m_pHead.load(memory_model::memory_order_relaxed) == h ) {
                            iter = pNext ;
    ...
    public:
            ...
            , m_nMaxHops( 3 )
        {
            ...
            static_assert(( boost::is_same<gc, typename node_type::gc>::value ), "GC and node_type::gc must be the same")    ;

            // For cds::gc::HRC, only one base_hook is allowed
            static_assert((
                boost::mpl::if_<
                    boost::is_same<gc, cds::gc::HRC>,
                    boost::is_same< typename hook::hook_type, opt::base_hook_tag >,
                    boost::true_type
                >::type::value
            ), "For cds::gc::HRC, only one base_hook is allowed") ;
            ...
            m_pHead.store( marked_ptr(&m_Dummy), memory_model::memory_order_relaxed ) ;
            m_pTail.store( marked_ptr(&m_Dummy), memory_model::memory_order_relaxed ) ;
        }
        ...
        */
        ~BasketQueue()
        {
            ...
            assert( pHead == m_pTail.load(memory_model::memory_order_relaxed).ptr() )    ;
        ...
        bool enqueue( value_type& val )
        {
            ...
            while ( true ) {
                ...
                try_again:
                    ...
                    // add to the basket
                    //while ( m_pTail.load(memory_model::memory_order_relaxed) == t && !pNext.bits()  ) {
        ...
        */
        ...
        /**
            Note that ithis function is not \p const.
