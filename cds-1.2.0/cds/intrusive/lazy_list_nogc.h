    ...
    protected:
        ...
        bool find_at( node_type * pHead, Q& val, Func f )
        {
            ...
            if ( pos.pCur != &m_Tail ) {
                ...
                if ( key_comparator()( *node_traits::to_value_ptr( *pos.pCur ), val ) == 0 )
                {
        ...
        template <typename Q>
        value_type * find_at( node_type * pHead, Q& val)
        {
            iterator it = find_at_( pHead, val )    ;
            if ( it != end() )
                return &*it ;
        ...
        template <typename Q>
        iterator find_at_( node_type * pHead, Q& val)
        {
            position pos    ;
