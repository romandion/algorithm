...
#define TEST_CASE( Q, V ) void Q() { test< Types<V>::Q >(); }
    ...
    protected:
        ...
        void analyze( CppUnitMini::ThreadPool& pool, QUEUE& testQueue  )
        {
            ...
            CPPUNIT_ASSERT_MSG( nTotalPops == nPoppedItems, "nTotalPops=" << nTotalPops << ", nPoppedItems=" << nPoppedItems ) ;
            ...
            CPPUNIT_ASSERT_MSG( nTotalPops + nPostTestPops == nQueueSize, "popped=" << nTotalPops + nPostTestPops << " must be " << nQueueSize ) ;
            ...
            for ( size_t nWriter = 0; nWriter < s_nWriterThreadCount; ++nWriter ) {
                ...
                CPPUNIT_ASSERT_MSG( arrData[0] == 0, "Writer " << nWriter << "[0] != 0" ) ;
                CPPUNIT_ASSERT_MSG( arrData[arrData.size() - 1] == m_nThreadPushCount - 1, "Writer " << nWriter << "[last] != " << m_nThreadPushCount - 1 ) ;
            }
        }
